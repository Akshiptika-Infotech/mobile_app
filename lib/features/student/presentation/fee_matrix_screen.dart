...
class FeeMatrixScreen extends ConsumerWidget {
  const FeeMatrixScreen({super.key});
    ...
    return Scaffold(
                ...
                : Padding(
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        ...
                        onChanged: (id) => ref
                            .read(selectedStudentYearIdProvider.notifier)
                            .state = id,
                        items: years
      ...
      body: feeAsync.when(
        ...
        error: (e, _) => _ErrorView(
          message: e.toString(),
          onRetry: () => ref.invalidate(studentFeeMatrixProvider),
        ),
        data: (matrix) {
          if (matrix == null) {
            return const Center(child: CircularProgressIndicator());
          }
          return _FeeBody(matrix: matrix, currency: currency);
        },
      ),
    );
  }
...
class _FeeBody extends ConsumerStatefulWidget {
  const _FeeBody({required this.matrix, required this.currency});
...
class _FeeBodyState extends ConsumerState<_FeeBody> {
  ...
  final Set<String> _selectedOneTimeIds = {}; // keyed by feeTypeId
  ...
  List<_MonthGroup> get _monthGroups {
    final monthly =
        widget.matrix.rows.where((r) => !r.isOneTime).toList();
  ...
  double get _selectedTotal {
    ...
    final oneTimeTotal = widget.matrix.rows
        ...
        .fold(0.0, (s, r) => s + (r.cells.isNotEmpty ? r.cells.first.net : 0.0));
    return monthsTotal + oneTimeTotal;
  }

  int get _selectedCount => _selectedMonths.length + _selectedOneTimeIds.length;
  ...
  Future<void> _startPayment() async {
    if (_selectedMonths.isEmpty && _selectedOneTimeIds.isEmpty) return;
    ...
    for (final row in widget.matrix.rows) {
      if (!row.isOneTime || !_selectedOneTimeIds.contains(row.feeTypeId)) continue;
    ...
    try {
      final repo = ref.read(studentPortalRepositoryProvider);
          ...
          children: [
            ...
            if (oneTime.isNotEmpty) ...[
              ...
              ...oneTime.map((row) => _OneTimeFeeCard(
                    ...
                    onToggle: (row.cells.isNotEmpty && !row.cells.first.isPaid)
                        ? () => setState(() {
                              if (_selectedOneTimeIds.contains(row.feeTypeId)) {
            ...
            if (monthGroups.isNotEmpty) ...[
              Row(
                children: [
                  const Expanded(child: _SectionHeader(label: 'Monthly Fees')),
              ...
              ...monthGroups.map((group) => _MonthGroupCard(
                    ...
                    currency: currency,
                    isSelected:
                        _selectedMonths.contains(group.monthYear),
...
class _MonthGroupCardState extends State<_MonthGroupCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final currency = widget.currency;
    final canSelect = widget.onToggle != null;
        ...
        children: [
                  ...
                  SizedBox(
                    ...
                    child: group.isFullyPaid
                        ...
                        : Checkbox(
                            value: isSelected,
                            onChanged:
                                canSelect ? (_) => widget.onToggle!() : null,
          ...
          if (_expanded) ...[
            Divider(
                ...
                indent: 16,
                endIndent: 16),
            ...group.entries.map((entry) => _FeeLineRow(
                  entry: entry,
                  currency: currency,
                )),
